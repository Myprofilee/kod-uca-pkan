...
(function() {
    const n = ()=>{
        ...
        const s = i=>{
            ...
            firebase.auth().sendSignInLinkToEmail(i, o).then(()=>{
                window.localStorage.setItem("MacaEmailSignIn", i),
                Notipin.Alert({
                    msg: a.successLine1,
                    type: "blue"
                }),
    ...
    if (firebase.auth().isSignInWithEmailLink(window.location.href)) {
        ...
        let i = window.localStorage.getItem("MacaEmailSignIn");
        i || Notipin.Prompt({
            msg: n.emailConfirm,
            onYes: i=>{
        ...
        firebase.auth().signInWithEmailLink(i, window.location.href).then(()=>{
            let n = window.localStorage.getItem("MacaEmailSignIn");
            n && window.localStorage.removeItem("MacaEmailSignIn"),
            window.location.href = `${window.location.origin}/meja/`
        }
        ).catch(i=>{
            Notipin.Alert({
